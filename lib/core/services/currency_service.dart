...
class CurrencyService {
  ...
  Future<void> loadSettings() async {
    ...
    if (settingsString != null && settingsString.isNotEmpty) {
      try {
        _currentSettings = CurrencySettings.fromJson(jsonDecode(settingsString));
      } catch (e) {
  ...
  double _convert(double amount, Currency fromCurrency, Currency toCurrency, CurrencySettings settings) {
    ...
    switch (toCurrency) {
      case Currency.USD:
        ...
        return settings.usdToCdfRate == 0 ? amountInCdf : amountInCdf / settings.usdToCdfRate;
      case Currency.FCFA:
        return settings.fcfaToCdfRate == 0 ? amountInCdf : amountInCdf / settings.fcfaToCdfRate;
      case Currency.CDF:
        return amountInCdf;
      // No default needed as all enum values are covered.
    }
  ...
  String formatAmount(double amount, {Currency? displayCurrency}) {
    ...
    double displayAmount = convertFromCdf(amount, targetCurrency); // Use the new method

    NumberFormat formatter;
    switch (targetCurrency) {
      case Currency.USD:
        // Example: \$1,234.56
        formatter = NumberFormat.currency(locale: 'en_US', symbol: targetCurrency.symbol, decimalDigits: 2);
        break;
      case Currency.FCFA:
        ...
        formatter = NumberFormat.currency(locale: 'fr_FR', symbol: targetCurrency.symbol, decimalDigits: 2);
        break;
      case Currency.CDF:
        // Example: 1.234,56 FC
        formatter = NumberFormat.currency(locale: 'fr_CD', symbol: targetCurrency.symbol, decimalDigits: 2); // fr_CD for Congo
        break;
      // No default needed as all enum values are covered.
    }
