...
class CustomerBloc extends Bloc<CustomerEvent, CustomerState> {
  ...
  CustomerBloc({required this.customerRepository}) : super(const CustomerInitial()) {
  ...
  Future<void> _onUpdateCustomer(
    ...
    try {
      final updatedCustomer = await customerRepository.updateCustomer(event.customer);
      emit(CustomerOperationSuccess(
  ...
  Future<void> _onDeleteCustomer(
    ...
    try {
      ...
      emit(const CustomerOperationSuccess(
  ...
  Future<void> _onSearchCustomers(
    ...
    try {
      if (event.searchTerm.isEmpty) {
        ...
        final searchResults = await customerRepository.searchCustomers(event.searchTerm);
        emit(CustomerSearchResults(
  ...
  Future<void> _onLoadTopCustomers(
    ...
    try {
      final topCustomers = await customerRepository.getTopCustomers(limit: event.limit);
      emit(TopCustomersLoaded(topCustomers));
    } catch (e) {
      emit(CustomerError('Erreur lors du chargement des meilleurs clients: $e'));
    }
  }
  ...
  Future<void> _onLoadRecentCustomers(
    ...
    try {
      final recentCustomers = await customerRepository.getRecentCustomers(limit: event.limit);
  ...
  Future<void> _onUpdateCustomerPurchaseTotal(
    ...
    try {
      ...
      emit(CustomerError('Erreur lors de la mise à jour du total des achats: $e'));
    }
  }
}
