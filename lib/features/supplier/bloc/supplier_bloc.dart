...
class SupplierBloc extends Bloc<SupplierEvent, SupplierState> {
  ...
  SupplierBloc({required this.supplierRepository}) : super(const SupplierInitial()) {
  ...
  Future<void> _onUpdateSupplier(
    ...
    try {
      final updatedSupplier = await supplierRepository.updateSupplier(event.supplier);
      emit(SupplierOperationSuccess(
  ...
  Future<void> _onSearchSuppliers(
    ...
    try {
      if (event.searchTerm.isEmpty) {
        ...
        final searchResults = await supplierRepository.searchSuppliers(event.searchTerm);
        emit(SupplierSearchResults(
  ...
  Future<void> _onLoadTopSuppliers(
    ...
    try {
      final topSuppliers = await supplierRepository.getTopSuppliers(limit: event.limit);
      emit(TopSuppliersLoaded(topSuppliers));
    } catch (e) {
      emit(SupplierError('Erreur lors du chargement des principaux fournisseurs: $e'));
    }
  }
  ...
  Future<void> _onLoadRecentSuppliers(
    ...
    try {
      final recentSuppliers = await supplierRepository.getRecentSuppliers(limit: event.limit);
      emit(RecentSuppliersLoaded(recentSuppliers));
    } catch (e) {
      emit(SupplierError('Erreur lors du chargement des fournisseurs récents: $e'));
    }
  }
  ...
  Future<void> _onUpdateSupplierPurchaseTotal(
    ...
    try {
      final updatedSupplier = await supplierRepository.updateSupplierPurchaseTotal(
        event.supplierId,
        event.amount,
      );
      ...
      emit(SupplierError('Erreur lors de la mise à jour du total des achats: $e'));
    }
  }
}
